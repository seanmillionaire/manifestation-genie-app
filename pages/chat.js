...
export default function ChatPage(){
  ...
  function pushHumanRecap() {
    ...
    const line = `Hey ${fn} — I’m with you. Your goal is “${wish}”, the snag is “${block}”, and your next tiny step is ${micro ? `“${micro}”` : 'set together now'}.`;
  ...
  async function heavyConfetti(){
    try {
      ...
      const end = Date.now() + 850; // slightly faster cleanup
      ...
      for (let i=0;i<3;i++){
        confetti({ particleCount: 160, startVelocity: 46, spread: 360, ticks: 90, origin: { y: 0.6 } });
  ...
  function triggerExerciseOne(){
    // Hand off to your existing exercise engine
  ...
  function onTweak() {  // <-- MISSING BEFORE (caused build failure)
    setShowTweaks(true);
  }
