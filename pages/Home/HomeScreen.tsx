...
type Profile = {
  firstName: string;
  agreements?: {
    manifestForGood?: { version: number; accepted: boolean; at: string };
  };
  onboarding?: { tipGuide?: { step: number; completedAt?: string } };
};
...
export default function HomeScreen() {
  ...
  const onTipStep = async (nextStep: number, completed?: boolean) => {
    setTipStep(nextStep);
    saveTipStep({ step: nextStep, completedAt: completed ? new Date().toISOString() : undefined }).catch(() => {});
  };
